...
<div class="min-h-screen bg-[#ECECEC] p-6">
    <div class="mb-6 flex items-center justify-between gap-4">
        <div>
            ...
            <p class="text-sm text-gray-600 mt-1">Projects assigned from Admin Project creation and Team Assign Roles.</p>
        </div>

        <form method="GET" action="{{ route('employee.projects.employeedashboard') }}" class="w-[420px] max-w-full">
            <div class="relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Search projects" class="w-full rounded-full border border-gray-400 bg-[#EDEDED] px-5 py-2 pr-11 text-sm outline-none">
    ...
    <div class="mt-6">
        ...
        @if($projects->isEmpty())
            <div class="rounded-2xl border border-gray-400 bg-[#E6E6E6] p-8 text-center text-gray-700">No assigned projects found.</div>
        @else
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
                @foreach($projects as $project)
                    @php
                        $progress = max(0, min(100, (int) ($project->progress ?? 0)));
                        $displayStatus = $progress >= 100 ? 'completed' : ($progress <= 0 ? 'pending' : 'ongoing');
                    @endphp
                    <a href="{{ route('employee.projects.show', $project->id) }}" class="group rounded-2xl overflow-hidden border border-black/10 bg-[#e7e7e7] shadow-[0_8px_18px_rgba(0,0,0,0.15)] transition hover:-translate-y-1">
                        <div class="p-3 pb-2">
                        ...
                        <div class="px-4 pb-4 pt-1">
                            ...
                            <div class="mt-3 flex items-center justify-between text-xs">
                                ...
                                <span class="font-semibold text-[#2b2b2b]">{{ ucfirst($displayStatus) }}</span>
                            </div>

                            <div class="mt-1.5 w-full bg-[#d2d2d2] rounded-full h-2 overflow-hidden">
                                <div class="bg-[#f6c915] h-2 rounded-full" style="width: {{ $progress }}%"></div>
                            </div>
                        </div>
                    </a>
                @endforeach
